...
column_file_path = '/home/kendra/Downloads/OSOH_survey_column_names_and_translations.csv'
...
def plot_categorical_column(column, regex_dict, ax, title = ' ', drop_other=False, kind='barh'):
...
matplotlib.rcParams.update({'font.size': 18})
df['language'].value_counts().plot(kind='barh', title='Language used in the survey')
...
plot_categorical_column(df['university'], uni_regexs, ax=ax, title = 'University', drop_other=False, kind='barh')
# -
...
plot_categorical_column(df['level'], level_regexs, ax=ax, 
                        title='Current level of training', 
                        drop_other=True, kind='pie')           
...
fig, ax = plt.subplots(figsize=(5,5))
plot_categorical_column(df['department'], dept_regexs, ax=ax, 
                        title='Department', 
                        drop_other=False, kind='pie')  

# ## What's your experience CREATING open-science objects? 

# +
create_use_cols = [col for col in list(df.columns) if "create_" in col or 'use_' in col]    
create_use_regexs = {
...
for key, regex_str in create_use_regexs.items():
    df_cleaned.replace(to_replace=regex_str, value=key, inplace=True, 
                       limit=None, regex=True, method='pad')
...
def ticklabels(var_list):
    return [s.replace('use_', '').replace('create_', '').replace('_', ' ').replace('each cit', 'each / cit') for s in var_list]
...
sns.heatmap(mat.loc[create_keys, create_cols].T, annot=True, vmax=len(df), ax=axs[0])
...
fig, ax = plt.subplots(figsize=(5,5))
plot_categorical_column(df['barriers'], barriers_regexs, ax=ax, 
...
def all_answers_wordcloud(column, title='', language='all'):
    ...
    wordcloud = WordCloud(max_font_size=50, max_words=100, background_color="white").generate(all_texts)
